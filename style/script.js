...
function saveData(data) {
  ...
  copy.auth.dynamicBase = encryptText(
    copy.auth.dynamicBase || '',
    key
  );
...
function applyTheme() {
  ...
  if (label) label.textContent = APP.settings.theme === 'dark' ? 'Dark Mode' : 'Light Mode';
  if (btn) btn.textContent = APP.settings.theme === 'dark' ? '🌙 Dark' : '☀️ Light';
}
...
function applyAccentColor() {
  if (APP.settings.accentColor) {
    document.documentElement.style.setProperty('--primary', APP.settings.accentColor);
    document.documentElement.style.setProperty('--primary-secondary', APP.settings.accentSecondary);
    document.documentElement.style.setProperty('--primary-glow', APP.settings.accentColor + '26');
  }
}
...
const COLORS = ['#ef4444','#f59e0b','#22c55e','#3b82f6','#8b5cf6','#ec4899','#06b6d4','#f97316',
                '#a855f7','#14b8a6','#f43f5e','#84cc16','#6366f1','#eab308','#0ea5e9','#d946ef'];
...
function switchLoginTab(tab) {
  ...
  document.querySelector(`.login-tab:nth-child(${tab === 'normal' ? 1 : 2})`).classList.add('active');
  document.getElementById(tab === 'normal' ? 'loginNormal' : 'loginCrazy').classList.add('active');
}

function switchCrazyMethod(method) {
  document.querySelectorAll('#loginCrazy .crazy-tab').forEach(t => t.classList.remove('active'));
  document.querySelectorAll('#loginCrazy .crazy-panel').forEach(p => p.classList.remove('active'));
  const methods = ['dot','emoji','color','dynamic'];
  const idx = methods.indexOf(method);
  document.querySelectorAll('#loginCrazy .crazy-tab')[idx].classList.add('active');
  document.getElementById('crazy' + method.charAt(0).toUpperCase() + method.slice(1)).classList.add('active');
...
function clearDotPattern() {
  currentDotPattern = [];
  document.querySelectorAll('#dotGrid .dot-cell').forEach(c => { c.classList.remove('selected'); c.textContent = ''; });
}

function submitDotLogin() {
  if (currentDotPattern.length < 3) return showToast('Pattern too short', 'error');
  if (!APP.auth.dotPattern.length) return showToast('No pattern set. Go to Settings first.', 'error');
  if (JSON.stringify(currentDotPattern) === JSON.stringify(APP.auth.dotPattern)) {
    loginSuccess();
  } else {
...
function clearEmojiPattern() {
  currentEmojiPattern = [];
  document.querySelectorAll('#emojiGrid .emoji-cell').forEach(c => c.classList.remove('selected'));
  document.getElementById('emojiSequence').innerHTML = '';
}

function submitEmojiLogin() {
  if (currentEmojiPattern.length < 3) return showToast('Sequence too short', 'error');
  if (!APP.auth.emojiPattern.length) return showToast('No sequence set. Go to Settings first.', 'error');
  if (JSON.stringify(currentEmojiPattern) === JSON.stringify(APP.auth.emojiPattern)) {
    loginSuccess();
  } else {
...
function clearColorPattern() {
  currentColorPattern = [];
  document.querySelectorAll('#colorGrid .color-cell').forEach(c => c.classList.remove('selected'));
  document.getElementById('colorSequence').innerHTML = '';
}

function submitColorLogin() {
  if (currentColorPattern.length < 3) return showToast('Pattern too short', 'error');
  if (!APP.auth.colorPattern.length) return showToast('No pattern set. Go to Settings first.', 'error');
  if (JSON.stringify(currentColorPattern) === JSON.stringify(APP.auth.colorPattern)) {
    loginSuccess();
  } else {
    showToast('Wrong pattern!', 'error');
    clearColorPattern();
...
function submitDynamicLogin() {
  ...
  if (!APP.auth.dynamicBase) return showToast('No base password set. Go to Settings first.', 'error');
  const now = new Date();
...
function normalLogin() {
  ...
  if (!APP.auth.username) return showToast('No account exists. Create one first.', 'error');
  if (user === APP.auth.username && simpleHash(pass) === APP.auth.passwordHash) {
    loginSuccess();
  } else {
...
function renderPage(page) {
  ...
  switch(page) {
    ...
    case 'secret-notes': renderSecretNotes(); break;
...
function renderDashboard() {
  ...
  const completedToday = APP.tasks.filter(t => t.completed && t.completedAt && t.completedAt.startsWith(todayStr)).length;
  const pending = APP.tasks.filter(t => !t.completed && !t.archived).length;
  const activeStreaks = APP.habits.filter(h => h.streak > 0).length;
  const bestStreak = APP.habits.reduce((max, h) => Math.max(max, h.bestStreak || 0), 0);
  ...
  // Today's tasks
  const todayTasks = APP.tasks.filter(t => !t.completed && !t.archived).slice(0, 5);
  const dashTasks = document.getElementById('dashTasks');
  if (!todayTasks.length) {
    dashTasks.innerHTML = '<div class="empty-state"><div class="empty-icon">✅</div><p>No pending tasks!</p></div>';
  } else {
    dashTasks.innerHTML = todayTasks.map(t => `
      <div class="task-item">
  ...
  if (!APP.habits.length) {
    dashHabits.innerHTML = '<div class="empty-state" style="padding:16px"><p>No habits yet</p></div>';
  } else {
    dashHabits.innerHTML = APP.habits.slice(0, 4).map(h => `
  ...
  if (!APP.longTermGoals.length) {
    dashGoals.innerHTML = '<div class="empty-state" style="padding:16px"><p>No goals yet</p></div>';
  } else {
    dashGoals.innerHTML = APP.longTermGoals.slice(0, 3).map(g => `
...
function renderDashChart() {
  ...
  const counts = days.map(d => APP.tasks.filter(t => t.completedAt && t.completedAt.startsWith(d)).length);
...
function renderTasks() {
  ...
  quadrants.forEach((q, i) => {
    ...
    if (taskFilter === 'today') tasks = tasks.filter(t => (t.deadline === todayStr || !t.deadline) && !t.archived);
    else if (taskFilter === 'completed') tasks = tasks.filter(t => t.completed);
    ...
    document.getElementById(countIds[i]).textContent = tasks.filter(t => !t.completed && !t.archived).length;

    if (!tasks.length) {
      container.innerHTML = '<div class="empty-state" style="padding:20px"><p style="font-size:0.8rem">No tasks</p></div>';
      return;
    }
...
// Drag & Drop
function dragTask(e, id) { e.dataTransfer.setData('taskId', id); }

document.querySelectorAll('.matrix-col').forEach(col => {
  col.addEventListener('dragover', e => { e.preventDefault(); col.style.borderColor = 'var(--primary)'; });
  ...
  col.addEventListener('drop', e => {
    ...
    if (task) { task.quadrant = q; saveData(APP); renderTasks(); }
  });
});
...
function archiveTask(id) {
  const t = APP.tasks.find(t => t.id === id);
  if (t) { t.archived = true; saveData(APP); renderTasks(); showToast('Task archived 📦'); }
}
...
function saveTask(editId) {
  ...
  if (editId) {
    ...
    if (t) { t.title = title; t.quadrant = quadrant; t.deadline = deadline; t.recurring = recurring; }
  } else {
    APP.tasks.push({ id: uid(), title, quadrant, completed: false, archived: false, deadline, recurring, createdAt: new Date().toISOString() });
  }
  saveData(APP);
  closeModal();
  renderTasks();
...
function renderHabits() {
  ...
  if (!APP.habits.length) {
    list.innerHTML = '<div class="empty-state"><div class="empty-icon">💪</div><p>No habits yet. Start building good habits!</p></div>';
    return;
  }
  const todayStr = today();
  list.innerHTML = APP.habits.map(h => {
...
function completeHabit(id) {
  ...
  const todayStr = today();
  if (h.completedDates.includes(todayStr)) return showToast('Already done today!');
...
function skipHabit(id) {
  ...
  if (!h.skippedDates) h.skippedDates = [];
  const todayStr = today();
  if (h.completedDates?.includes(todayStr) || h.skippedDates.includes(todayStr)) return;
...
function saveHabit(editId) {
  ...
  if (editId) {
    ...
    if (h) { h.title = title; h.frequency = frequency; h.points = points; }
  } else {
    APP.habits.push({ id: uid(), title, frequency, points, streak: 0, bestStreak: 0, completedDates: [], skippedDates: [], createdAt: new Date().toISOString() });
  }
  saveData(APP);
  closeModal();
  renderHabits();
...
function renderNotesSidebar() {
  ...
  if (!APP.generalNotes.length) {
    sidebar.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-muted);font-size:0.8rem">No notes yet</div>';
    return;
  }
  sidebar.innerHTML = APP.generalNotes.map(n => `
...
function updateNoteColor(color) {
  if (!currentNoteId) return;
  const n = APP.generalNotes.find(n => n.id === currentNoteId);
  if (n) { n.color = color; saveData(APP); renderNotesSidebar(); }
}
...
let secretDotPat = [], secretEmojiPat = [], secretColorPat = [];

function secretDotClick(i, cell) { secretDotPat.push(i); cell.classList.add('selected'); cell.textContent = secretDotPat.length; }
function clearSecretDot() { secretDotPat = []; document.querySelectorAll('#secretDotGrid .dot-cell').forEach(c => { c.classList.remove('selected'); c.textContent = ''; }); }
function submitSecretDot() {
  if (!APP.auth.dotPattern.length) return showToast('No pattern set. Go to Settings.', 'error');
  if (JSON.stringify(secretDotPat) === JSON.stringify(APP.auth.dotPattern)) { secretNotesUnlocked = true; renderSecretNotes(); showToast('Vault unlocked! 🔓'); }
  else { showToast('Wrong pattern!', 'error'); clearSecretDot(); }
}

function secretEmojiClick(i, em, cell) { secretEmojiPat.push(i); cell.classList.add('selected'); const s = document.getElementById('secretEmojiSeq'); if(s) s.innerHTML += `<span style="font-size:1.2rem">${em}</span>`; }
function clearSecretEmoji() { secretEmojiPat = []; document.querySelectorAll('#secretEmojiGrid .emoji-cell').forEach(c => c.classList.remove('selected')); const s = document.getElementById('secretEmojiSeq'); if(s) s.innerHTML = ''; }
function submitSecretEmoji() {
  if (!APP.auth.emojiPattern.length) return showToast('No sequence set. Go to Settings.', 'error');
  if (JSON.stringify(secretEmojiPat) === JSON.stringify(APP.auth.emojiPattern)) { secretNotesUnlocked = true; renderSecretNotes(); showToast('Vault unlocked! 🔓'); }
  else { showToast('Wrong sequence!', 'error'); clearSecretEmoji(); }
}

function secretColorClick(i, c, cell) { secretColorPat.push(i); cell.classList.add('selected'); const s = document.getElementById('secretColorSeq'); if(s) s.innerHTML += `<span style="display:inline-block;width:20px;height:20px;border-radius:4px;background:${c}"></span>`; }
function clearSecretColor() { secretColorPat = []; document.querySelectorAll('#secretColorGrid .color-cell').forEach(c => c.classList.remove('selected')); const s = document.getElementById('secretColorSeq'); if(s) s.innerHTML = ''; }
function submitSecretColor() {
  if (!APP.auth.colorPattern.length) return showToast('No pattern set. Go to Settings.', 'error');
  if (JSON.stringify(secretColorPat) === JSON.stringify(APP.auth.colorPattern)) { secretNotesUnlocked = true; renderSecretNotes(); showToast('Vault unlocked! 🔓'); }
  else { showToast('Wrong pattern!', 'error'); clearSecretColor(); }
}

function submitSecretDynamic() {
  const input = document.getElementById('secretDynInput').value;
  if (!APP.auth.dynamicBase) return showToast('No base password set. Go to Settings.', 'error');
  const now = new Date();
  const hhmm = now.getHours().toString().padStart(2,'0') + now.getMinutes().toString().padStart(2,'0');
  if (input === hhmm + APP.auth.dynamicBase) { secretNotesUnlocked = true; renderSecretNotes(); showToast('Vault unlocked! 🔓'); }
...
function renderSecretNotesSidebar() {
  ...
  if (!APP.secretNotes.length) {
    sidebar.innerHTML = '<div style="text-align:center;padding:20px;color:var(--text-muted);font-size:0.8rem">No secret notes yet</div>';
    return;
  }
  sidebar.innerHTML = APP.secretNotes.map(n => `
...
function selectSecretNote(id) {
  ...
  document.getElementById('secretEditorContent').innerHTML = n.encrypted ? decryptText(n.content, key) : (n.content || '');
  renderSecretNotesSidebar();
}

function createNewSecretNote() {
  const note = { id: uid(), title: 'Secret Note', content: '', encrypted: true, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
...
function autoSaveSecretNote() {
  ...
  secretAutoSaveTimer = setTimeout(() => {
    ...
    const key = APP.auth.secretKey || '';
    n.content = encryptText(document.getElementById('secretEditorContent').innerHTML, key);
    n.encrypted = true;
    n.updatedAt = new Date().toISOString();
    saveData(APP);
    renderSecretNotesSidebar();
...
function renderGoals() {
  ...
  if (!APP.longTermGoals.length) {
    list.innerHTML = '<div class="empty-state"><div class="empty-icon">🎯</div><p>No goals yet. Set your first long-term goal!</p></div>';
    return;
  }
  list.innerHTML = APP.longTermGoals.map(g => `
    <div class="goal-item">
...
function adjustGoalProgress(id, delta) {
  ...
  if (g.progress === 100) { APP.totalPoints += 50; logPoints(50, 'Goal completed'); showToast('Goal completed! +50 points! 🏆'); }
  saveData(APP);
  renderGoals();
  renderDashboard();
}
...
function saveGoal(editId) {
  ...
  const progress = Math.max(0, Math.min(100, parseInt(document.getElementById('goalProgress').value) || 0));

  if (editId) {
    const g = APP.longTermGoals.find(g => g.id === editId);
    if (g) { g.title = title; g.deadline = deadline; g.progress = progress; }
  } else {
    APP.longTermGoals.push({ id: uid(), title, deadline, progress, createdAt: new Date().toISOString() });
  }
  saveData(APP);
  closeModal();
  renderGoals();
...
function logPoints(pts, reason) {
  ...
  APP.pointsHistory.push({ date: new Date().toISOString(), points: pts, reason });
  if (APP.pointsHistory.length > 500) APP.pointsHistory = APP.pointsHistory.slice(-500);
}

function renderAnalytics() {
  document.getElementById('anPoints').textContent = APP.totalPoints;
  document.getElementById('anTasksDone').textContent = APP.tasks.filter(t => t.completed).length;
  document.getElementById('anHabitsCount').textContent = APP.habits.length;
  document.getElementById('anPomSessions').textContent = APP.pomodoroSessions || 0;
...
function renderTaskCompletionChart() {
  ...
  const counts = days.map(d => APP.tasks.filter(t => t.completedAt && t.completedAt.startsWith(d)).length);
  const max = Math.max(...counts, 1);
  const colors = ['#3b82f6','#6366f1','#8b5cf6','#a855f7','#ec4899','#f43f5e','#ef4444'];
...
function renderHabitStreaksChart() {
  ...
  if (!APP.habits.length) {
    container.innerHTML = '<div class="empty-state" style="padding:20px"><p>No habits</p></div>';
    return;
  }
  const max = Math.max(...APP.habits.map(h => h.bestStreak || 0), 1);
  const colors = ['#22c55e','#f59e0b','#3b82f6','#8b5cf6','#ef4444','#ec4899','#06b6d4','#f97316'];
  container.innerHTML = APP.habits.slice(0, 8).map((h, i) => {
...
function renderProductivityScore() {
  ...
  const todayStr = today();
  const tasksToday = APP.tasks.filter(t => t.completedAt && t.completedAt.startsWith(todayStr)).length;
  const habitsToday = APP.habits.filter(h => h.completedDates?.includes(todayStr)).length;
  const totalHabits = APP.habits.length || 1;
  const score = Math.min(100, Math.round(((tasksToday * 10) + (habitsToday / totalHabits * 50) + Math.min(APP.pomodoroSessions || 0, 4) * 10)));
...
function renderPointsHistoryChart() {
  ...
  const pointsByDay = days.map(d => history.filter(p => p.date.startsWith(d)).reduce((sum, p) => sum + p.points, 0));
  const max = Math.max(...pointsByDay, 1);
  const dayNames = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
  container.innerHTML = pointsByDay.map((p, i) => {
    const pct = (p / max) * 100;
    const dayIdx = new Date(days[i]).getDay();
...
function renderCalendar() {
  ...
  let html = dayHeaders.map(d => `<div class="cal-header-cell">${d}</div>`).join('');
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const tasks = APP.tasks.filter(t => t.deadline === dateStr || (t.completedAt && t.completedAt.startsWith(dateStr)));
...
function showCalDay(dateStr) {
  ...
  let html = '';
  if (tasks.length) {
    html += '<h4 style="font-size:0.8rem;color:var(--text-muted);margin-bottom:8px">TASKS</h4>';
    html += tasks.map(t => `<div class="task-item"><div class="task-info"><div class="task-title">${esc(t.title)} ${t.completed ? '✅' : ''}</div></div></div>`).join('');
  }
  if (habits.length) {
    html += '<h4 style="font-size:0.8rem;color:var(--text-muted);margin:12px 0 8px">HABITS COMPLETED</h4>';
    html += habits.map(h => `<div class="task-item"><div class="task-info"><div class="task-title">✅ ${esc(h.title)}</div></div></div>`).join('');
  }
  if (!html) html = '<div class="empty-state" style="padding:20px"><p>Nothing on this day</p></div>';
...
function renderPomodoro() {
  ...
  document.getElementById('pomodoroSessions').textContent = APP.pomodoroSessions || 0;
  if (!pomRunning) {
    pomSeconds = mins * 60;
...
function togglePomodoro() {
  if (pomRunning) {
    ...
    document.getElementById('pomodoroCircle').classList.remove('running', 'break');
  } else {
    if (pomSeconds <= 0) pomSeconds = (APP.pomodoroMinutes || 25) * 60;
    pomRunning = true;
    document.getElementById('pomodoroStartBtn').textContent = 'Pause';
    document.getElementById('pomodoroCircle').classList.add(pomBreak ? 'break' : 'running');
...
document.getElementById('pomodoroMinInput').addEventListener('change', function() { updatePomodoroSetting(this.value); });

// ===== SETTINGS =====
function renderSettings() {
  const isLocked = getAppLock();

  document.getElementById('settingsAppLock').textContent = isLocked ? 'On' : 'Off';
  document.getElementById('settingsSecretMethod').value = APP.settings.secretMethod || 'dot';
  document.getElementById('settingsDefaultPoints').value = APP.settings.defaultHabitPoints || 5;
  document.getElementById('settingsBreakMin').value = APP.settings.breakMinutes || 5;
...
function openSetPattern(type) {
  ...
  setTimeout(() => {
    if (type === 'dot') {
      settingDotPat = [];
      initDotGrid('setDotGrid', (i, cell) => { settingDotPat.push(i); cell.classList.add('selected'); cell.textContent = settingDotPat.length; });
    } else if (type === 'emoji') {
      settingEmojiPat = [];
      initEmojiGrid('setEmojiGrid', (i, em, cell) => {
        settingEmojiPat.push(i); cell.classList.add('selected');
        document.getElementById('setEmojiSeq').innerHTML += `<span style="font-size:1.2rem">${em}</span>`;
      });
    } else if (type === 'color') {
      settingColorPat = [];
      initColorGrid('setColorGrid', (i, c, cell) => {
        settingColorPat.push(i); cell.classList.add('selected');
        document.getElementById('setColorSeq').innerHTML += `<span style="display:inline-block;width:20px;height:20px;border-radius:4px;background:${c}"></span>`;
      });
    }
  }, 50);
}

let settingDotPat = [], settingEmojiPat = [], settingColorPat = [];

function saveDotPattern() {
  if (settingDotPat.length < 3) return showToast('Min 3 dots required', 'error');
...
function saveEmojiPattern() {
  if (settingEmojiPat.length < 3) return showToast('Min 3 emojis required', 'error');
...
function saveColorPattern() {
  if (settingColorPat.length < 3) return showToast('Min 3 colors required', 'error');
...
document.getElementById('globalSearch').addEventListener('input', function() {
  ...
  const taskResults = APP.tasks.filter(t => t.title.toLowerCase().includes(q));
  const noteResults = APP.generalNotes.filter(n => n.title.toLowerCase().includes(q) || (n.content || '').toLowerCase().includes(q));
  ...
  if (noteResults.length) {
    html += '<h4 style="font-size:0.8rem;color:var(--text-muted);margin:12px 0 8px">NOTES</h4>';
  ...
  APP.tasks.forEach(t => {
    if (t.recurring && t.completed && t.completedAt) {
      ...
      if (t.recurring === 'weekly') {
        const daysDiff = (new Date(todayStr) - new Date(completedDate)) / 86400000;
        if (daysDiff >= 7) { t.completed = false; t.completedAt = undefined; }
      }
    }
  });
  saveData(APP);
